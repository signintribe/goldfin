...
@Component({
  selector: 'app-laf-form4',
  templateUrl: './laf-form4.component.html',
  styleUrls: ['./laf-form4.component.css']
})
export class LafForm4Component implements OnInit {
  data = <any>{};
  data1 = <any>{};
  data3 = <any>{};
  sucess = "";
  ...
  delDocument(data : any){
    let obj = {
      key : data,
      _id : this.userId
    }
    this._auth.deleteDocuments(obj).subscribe(
      res => {
        if(res.status==200){
          alert("Selected Document has been deleted");
        }
        else {
  ...
  delDocument1(data : any){
    let obj = {
      key : data,
      _id : this.userId
    }
    this._auth.deleteDocuments1(obj).subscribe(
      res => {
        if(res.status==200){
          alert("Selected Document has been deleted");
        }
